...
How **consentledger** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `consentledger` lives in the **Cloud / supply-chain hardening** cluster.
...
| from | relation | to |
|---|---|---|
| `consentledger` | JSON findings compose with | [`sbomgen`](https://github.com/cognis-digital/sbomgen), [`cvecheck`](https://github.com/cognis-digital/cvecheck), [`k8sharden`](https://github.com/cognis-digital/k8sharden), [`tfscan`](https://github.com/cognis-digital/tfscan) |
| `consentledger` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `consentledger` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `consentledger` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the cloud / supply-chain hardening cluster.** `consentledger` output feeds the next tool:
```bash
consentledger ... --format json > out.json          # this tool's findings (see `consentledger --help`)
...
consentledger ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
consentledger ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
consentledger ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `consentledger`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `consentledger` slots
