"""Native cognis-connect emit for consentledger — forward findings to any platform.

Maps consentledger's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    consentledger ... --format json | consentledger-emit --to stix
    consentledger-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out.pop('ts', None)
        out.pop('actor', None)
        out.pop('patient', None)
        out.pop('action', None)
        out.pop('resource', None)
        out.pop('meta', None)
        out['title'] = rec.get('resource', '') + ' ' + rec.get('action', '')
        out['severity'] = 'info'
        out['type'] = 'access'
        out['description'] = f"{rec.get('actor', '')} {rec.get('action', '')} {rec.get('resource', '')}"
        out['tags'] = [rec.get('patient', ''), rec.get('resource', '')]
